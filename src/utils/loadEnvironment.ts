...
class LoadEnvironment {
  ...
  static getInfo(varName: string): ObjectType {                     //Devuelve el valor de una variable de entorno desde un archivo jso// En string recoge la variable de consola
  ...
  private static getEnvironmentInfo(): LoadEnvironment {           //Cargar configuracion de entorno  si no esta cargado leel el Json del entorno
    if (!LoadEnvironment.info) {                                   // Verifica si LoadEnviroment.info ya esta cargado
  ...
  static getArguments(): ObjectType {  //usa minimist para leer argumentos pasados al ejecutar el script
   ...
   */
  static getFixture(localPath: string): ObjectType { //carga archivos de prueba
    const { language } = this.getArguments();
    if (!language) {
      console.error('Language has not been set. Please, indicate the language in the command as a parameter. '  //Si no tiene idioma para la ejecucion y muestra un error
