...
class HarryValidation {
  ...
  /**
   * @description Revisar que la resspuesta contiene las propiedades indicadas y sea de typo array
   */
  public chekKeyInJson = (harryInfo: ObjectType): void => {
 ...
 public chekEmptyOrNull = (harryInfo: ObjectType): void => {
    harryInfo.forEach((item: any) => {
    assert.notEmpty(item.number, "El campo 'number' no puede estar vacío");
      assert.notEmpty(item.title, "El campo 'title' no puede estar vacío");
      assert.notEmpty(item.originalTitle, "El campo 'originalTitle' no puede estar vacío");
      assert.notEmpty(item.releaseDate, "El campo 'releaseDate' no puede estar vacío");
      assert.notEmpty(item.description, "El campo 'description' no puede estar vacío");
      assert.notEmpty(item.pages, "El campo 'pages' no puede estar vacío");
      assert.notEmpty(item.cover, "El campo 'cover' no puede estar vacío");
      assert.notEmpty(item.index, "El campo 'index' no puede estar vacío");
    });
 }
